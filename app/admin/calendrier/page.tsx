...
const getCalendarData=cache(async()=>{
  ...
  const[{data:reservations},{data:requests},{data:blocks},{data:prices},{data:seasons},{data:settings}]=await Promise.all([
    db.from("reservations").select("id,reference,check_in,check_out,source,status,guest_first_name,guest_last_name,total").lt("check_in",to).gt("check_out",from).in("status",["confirmed","pending_payment"]),
  ...
  const events:CalendarEvent[]=[
    ...(reservations??[]).map(row=>({id:row.id,start:row.check_in,end:row.check_out,label:row.reference,source:row.source==="booking"?"booking":row.source==="airbnb"?"airbnb":row.status==="pending_payment"?"payment":"confirmed",kind:"reservation" as const,status:row.status,guest:`${row.guest_first_name} ${row.guest_last_name}`,total:row.total})),
