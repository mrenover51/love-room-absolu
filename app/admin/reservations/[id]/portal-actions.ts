...
const idSchema=z.uuid(),typeSchema=z.enum(['confirmation','pre_arrival','access_48h','access_ready','checkout_reminder','post_stay_review']);
...
export async function saveReservationAccess(formData:FormData){await requireAdmin();const input=z.object({id:z.uuid(),keyboxCode:z.union([z.literal(''),z.string().trim().min(3).max(32)])}).parse(Object.fromEntries(formData));const{error}=await createAdminClient().from('reservations').update({keybox_code:input.keyboxCode||null,updated_at:new Date().toISOString()}).eq('id',input.id);if(error)throw new Error('Accès impossible à enregistrer');await auditAdminAction('guest_portal.access_update','reservation',input.id,{code:input.keyboxCode?'configured':'default'});revalidatePath(`/admin/reservations/${input.id}`)}
