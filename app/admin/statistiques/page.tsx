...
export default async function Statistics() {
  ...
  const db=createAdminClient(),[{ data },{data:customers}]=await Promise.all([db.from("reservations").select("created_at,check_in,nights,total,source,payment_status,status").gte("check_in", `${year - 1}-01-01`).lte("check_in", `${year}-12-31`),db.from("customers").select("city,country")]);
  ...
  return <>
    ...
    <StatisticsDashboard rows={rows} year={year} customerOrigins={customers??[]} />
  </>;
}
