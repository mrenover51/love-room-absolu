...
const schema = z.object({
  ...
  metadata: z
    .record(z.string(), z.union([z.string().max(100), z.number(), z.boolean()]))
    .optional(),
});
const attempts = new Map<string, number[]>();
function channel(referrer?: string) {
...
export async function POST(request: Request) {
  ...
  const ip = request.headers.get("x-forwarded-for")?.split(",")[0] ?? "local",
    now = Date.now(),
    recent = (attempts.get(ip) ?? []).filter((time) => now - time < 60_000);
  ...
  try {
    ...
    await createAdminClient()
      ...
      .insert({
        ...
        event_name: d.event,
        path: d.path.split("?")[0],
        referrer_host: d.referrer
          ? new URL(d.referrer).hostname.slice(0, 200)
          : null,
        channel: channel(d.referrer),
        value: d.value ?? null,
        scroll_depth: d.scrollDepth ?? null,
        x_percent: d.x ?? null,
        y_percent: d.y ?? null,
        metadata: d.metadata ?? {},
      });
