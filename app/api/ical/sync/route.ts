...
async function sync(request:Request){if(!await authorized(request))return Response.json({error:"Non autorisé"},{status:401});const results=await syncAllCalendars();return Response.json({ok:results.every(item=>item.status!=="failed"),generatedAt:new Date().toISOString(),results},{status:results.some(item=>item.status==="failed")?207:200})}
