...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(await new CheckoutService().create(parsed.data), { status: 201 });
  } catch (error) {
    const code = error instanceof Error ? error.message : "UNKNOWN";
    if (code.includes("DATES_UNAVAILABLE")) return NextResponse.json({ error: "Ces dates ne sont plus disponibles.", code: "DATES_UNAVAILABLE" }, { status: 409 });
    ...
    return NextResponse.json({ error: "Le paiement n’a pas pu être préparé. Réessayez." }, { status: 500 });
  }
}
