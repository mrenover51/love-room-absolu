...
export default async function AnswerPage({params}:{params:Promise<{question:string}>}){
  ...
  return <><Header/><main><JsonLd data={schema}/>
    ...
    <section className="bg-[#F2ECE4] py-20 text-[#201B18]"><div className="page-shell"><aside id="reponse-directe" className="rounded-[2rem] border border-[#8B6B36]/25 bg-white p-8"><p className="eyebrow text-[#8B6B36]">Réponse courte</p><p className="mt-5 max-w-5xl text-xl leading-9">{item.shortAnswer}</p></aside>
      <article className="mx-auto mt-20 max-w-4xl"><h2 className="font-heading text-5xl">Définition</h2><p className="mt-6 text-lg leading-8 text-black/60">{item.definition}</p><h2 className="mt-16 font-heading text-5xl">Réponse détaillée</h2><div className="mt-7 space-y-6 text-lg leading-8 text-black/60">{item.longAnswer.map(text=><p key={text}>{text}</p>)}</div>
        ...
        <section id="faq"><h2 className="mt-16 font-heading text-5xl">Questions associées</h2><div className="mt-7 divide-y divide-black/10">{item.faq.map(entry=><details key={entry.question}><summary className="cursor-pointer py-5 font-heading text-2xl">{entry.question}</summary><p className="pb-6 leading-8 text-black/60">{entry.answer}</p></details>)}</div></section>
      </article></div></section>
