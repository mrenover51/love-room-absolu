...
import { Bath, Flame, Flower2, HeartHandshake, Lightbulb, Music2, Radio, Sparkles } from "lucide-react";
import { SectionHeading } from "@/components/shared/section-heading";
import { theme } from "@/lib/theme";

const moments = [{ label: "Rituel de relaxation à deux", icon: HeartHandshake }, { label: "Massage en duo", icon: Sparkles }, { label: "Ambiance lumineuse tamisée", icon: Lightbulb }, { label: "Bain balnéo romantique", icon: Bath }, { label: "Sauna privatif", icon: Radio }, { label: "Musique d’ambiance", icon: Music2 }, { label: "Bougies LED", icon: Flame }, { label: "Temps de déconnexion", icon: Flower2 }] as const;
...
export function TantraExperience() {
  ...
  return <section className="relative isolate overflow-hidden bg-[#090909] py-24 text-[#F6F2EC] sm:py-32" aria-labelledby="tantra-title">
    ...
    <div className="page-shell relative"><motion.div initial={reducedMotion ? false : { opacity: 0, y: 20 }} whileInView={reducedMotion ? undefined : { opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: theme.durations.reveal, ease: [0.22, 1, 0.36, 1] }} className="grid gap-8 rounded-[2rem] border border-white/10 bg-white/[.055] p-6 shadow-[0_30px_100px_rgba(0,0,0,.35)] backdrop-blur-xl sm:p-10 lg:grid-cols-[.72fr_1.28fr] lg:gap-16 lg:p-14">
      <div><span className="mb-7 grid size-14 place-items-center rounded-2xl border border-[#C9A86A]/25 bg-[#C9A86A]/10"><Flower2 className="size-6 text-[#E5C98E]" aria-hidden="true" /></span><SectionHeading eyebrow="Bien-être à deux" title="Expérience Tantra" light /><p id="tantra-title" className="mt-6 max-w-xl text-sm leading-7 text-white/60 sm:text-base sm:leading-8">Un espace pensé pour ralentir, se reconnecter et partager un moment privilégié à deux. L&apos;ambiance de la suite favorise la détente, la respiration, le massage et les rituels de bien-être inspirés de l&apos;univers du tantra.</p></div>
