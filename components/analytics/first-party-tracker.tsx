...
function session() {
  let id = sessionStorage.getItem("absolu-analytics-session");
  if (!id) {
    id = crypto.randomUUID();
    sessionStorage.setItem("absolu-analytics-session", id);
  }
  return id;
}
  ...
  useEffect(() => {
    ...
    function scroll() {
      const max = document.documentElement.scrollHeight - innerHeight,
        depth = max <= 0 ? 100 : Math.round((scrollY / max) * 100);
      for (const threshold of thresholds)
        if (depth >= threshold && !reached.has(threshold)) {
          reached.add(threshold);
          send("scroll", { scrollDepth: threshold });
        }
    }
    function click(event: MouseEvent) {
      ...
      if (href.startsWith("/reservation"))
        send("reserve_click", { metadata: { destination: "/reservation" } });
      else if (href.startsWith("tel:")) send("contact_phone_click");
