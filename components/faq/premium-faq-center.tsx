...
export function PremiumFaqCenter({ items, categories }: { items: PremiumFaqItem[]; categories: readonly FaqCategory[] }) {
  ...
  return <div>
    <div className="rounded-[2rem] border border-black/10 bg-white p-5 shadow-sm sm:p-8">
      ...
      <div className="mt-6 flex flex-wrap gap-2" aria-label="Filtres éditoriaux">{(["Toutes", "Populaires", "Récentes"] as View[]).map((option, keyIndex) => <button type="button" key={`${option}-${keyIndex}`} onClick={() => { setView(option); setLimit(12); }} aria-pressed={view === option} className={`rounded-full px-4 py-2 text-xs transition ${view === option ? "bg-[#201B18] text-white" : "border border-black/15 hover:border-black/35"}`}>{option === "Toutes" ? "Toutes les questions" : `Questions ${option.toLowerCase()}`}</button>)}</div>
    </div>
    <nav aria-label="Catégories de questions" className="mt-8 flex gap-2 overflow-x-auto pb-3">{(["Toutes", ...categories] as const).map((option, keyIndex) => <button type="button" key={`${option}-${keyIndex}`} onClick={() => updateCategory(option)} aria-pressed={category === option} className={`shrink-0 rounded-full px-4 py-2 text-sm transition ${category === option ? "bg-[#8B6B36] text-white" : "border border-black/15 bg-white hover:border-[#8B6B36]"}`}>{option}</button>)}</nav>
