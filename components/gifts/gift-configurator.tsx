...
export function GiftConfigurator({ theme }: { theme: GiftTheme }) {
  ...
  return <section id="personnaliser" className="relative isolate overflow-hidden py-24">
    <Image src="/images/optimized/lit.avif" alt="" fill sizes="100vw" className="gift-card-background -z-20 object-cover" />
    <div className="absolute inset-0 -z-10 bg-black/55" />
    <div className="page-shell">
      <div className="mx-auto mb-12 max-w-3xl text-center">
        <h2 className="font-heading text-5xl sm:text-6xl">Offrez une parenthèse inoubliable</h2>
        <p className="mx-auto mt-5 max-w-2xl leading-8 text-white/70">Créez un bon cadeau personnalisé pour faire vivre une expérience romantique au cœur de la Champagne.</p>
      </div>
      <div className="grid gap-8 lg:grid-cols-[.9fr_1.1fr]">
    <form onSubmit={checkout} className="rounded-[2rem] border border-white/20 bg-black/55 p-6 shadow-2xl backdrop-blur-xl sm:p-8">
      ...
      <fieldset className="mt-6"><legend className="text-xs text-white/55">Montant</legend><div className="mt-2 grid grid-cols-3 gap-2">{giftAmounts.map((value, keyIndex)=><button type="button" key={`${value}-${keyIndex}`} onClick={()=>setAmount(value)} className={`min-h-12 rounded-xl border text-sm ${amount===value?"border-[#C9A86A] bg-[#C9A86A] text-black":"border-white/10"}`}>{value} €</button>)}</div></fieldset>
    ...
    <div className="lg:sticky lg:top-28 lg:self-start"><p className="mb-3 text-xs uppercase tracking-[.2em] text-white/35">Aperçu avant achat</p>
      <motion.article initial={reduced?false:{opacity:0,y:12}} animate={{opacity:1,y:0}} className="relative aspect-[1.414/1] overflow-hidden rounded-[2rem] border border-white/15 p-7 shadow-2xl sm:p-10">
        {!photo&&<Image src={theme.image} alt="" fill sizes="(max-width: 1024px) 100vw, 58vw" className="object-cover"/>}{photo&&<div role="img" aria-label="Photo personnelle du bon cadeau" className="absolute inset-0 bg-cover bg-center" style={{backgroundImage:`url(${photo})`}}/>}<div className="absolute inset-0" style={{background:`linear-gradient(135deg, ${color}66, rgba(9,9,9,.82) 62%)`}}/><div className="relative flex h-full flex-col"><Gift className="size-7" style={{color}}/><p className="mt-5 text-[10px] uppercase tracking-[.3em]" style={{color}}>Love Room Absolu · Avize</p><h3 className="mt-2 font-heading text-4xl sm:text-6xl">{theme.shortName}</h3><p className="mt-4 line-clamp-3 max-w-lg text-sm italic text-white/70">« {message} »</p><div className="mt-auto flex items-end justify-between"><div><p className="text-sm">Pour {recipient||"votre moitié"}</p><p className="mt-1 text-xs text-white/45">De la part de {sender||"vous"} · valable 12 mois après activation</p></div><strong className="font-heading text-4xl">{amount} €</strong></div></div>
      </motion.article>
