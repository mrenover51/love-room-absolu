...
export function BookingFlow({pricingConfig}:{pricingConfig:PublicPricingConfig}) {
  ...
  const pricing=useMemo(()=>{try{return values.checkIn&&values.checkOut?calculatePriceFromConfig(values.checkIn,values.checkOut,values.extraKeys??[],pricingConfig):null}catch{return null}},[values.checkIn,values.checkOut,values.extraKeys,pricingConfig]);
  ...
  return <form onSubmit={form.handleSubmit(submit)} noValidate><ol className="mb-10 grid grid-cols-4 gap-2" aria-label="Étapes de réservation">{steps.map((label,index)=><li key={label} aria-current={index===step?"step":undefined} className={`border-t pt-3 text-center text-[.58rem] uppercase tracking-wider sm:text-[.68rem] ${index<=step?"border-[#C9A86A] text-[#D8C8B6]":"border-white/10 text-white/30"}`}>{index+1}. {label}</li>)}</ol><div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px]"><div className="min-w-0 border border-white/10 bg-black/25 p-5 sm:p-8"><h2 ref={headingRef} tabIndex={-1} className="sr-only">Étape {step+1} : {steps[step]}</h2>
    {step===0&&<DateRangePicker checkIn={values.checkIn} checkOut={values.checkOut} minimumNights={pricingConfig.minimumNights} maximumNights={pricingConfig.maximumNights} onChange={(checkIn,checkOut)=>{form.setValue("checkIn",checkIn,{shouldValidate:true});form.setValue("checkOut",checkOut,{shouldValidate:true})}}/>}
    {step===1&&<ExtrasSelector selected={values.extraKeys??[]} extras={pricingConfig.extras} onChange={(extraKeys)=>form.setValue("extraKeys",extraKeys,{shouldValidate:true})}/>} 
