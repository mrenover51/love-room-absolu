...
export function parseIcal(content: string, source: CalendarSource = "booking"): ExternalCalendarEvent[] {
...
async function fetchCalendar(source: CalendarSource, url: string) {
  ...
  if (previous && previous.expiresAt > Date.now()) return { events: previous.events, status: "ok" as const };
  try {
    const response = await fetch(url, { cache: "no-store", signal: AbortSignal.timeout(8_000), headers: { accept: "text/calendar" } });
    ...
    console.warn("ical_sync_failed", { source, code: error instanceof Error ? error.message.slice(0, 40) : "UNKNOWN" });
    return { events: previous?.events ?? [], status: "error" as const };
  }
}

export async function getExternalCalendarData() {
  const definitions: Array<[CalendarSource, string | undefined]> = [["booking", process.env.BOOKING_ICAL_URL??process.env.BOOKING_ICAL], ["airbnb", process.env.AIRBNB_ICAL_URL??process.env.AIRBNB_ICAL]];
  const results = await Promise.all(definitions.map(async ([source, url]) => url ? fetchCalendar(source, url) : ({ events: [], status: "not_configured" as const })));
  const events = [...new Map(results.flatMap((result) => result.events).map((event) => [`${event.start}:${event.end}:${event.uid}`, event])).values()];
  return { events, statuses: { booking: results[0].status, airbnb: results[1].status }, synchronizedAt: new Date().toISOString() };
}
