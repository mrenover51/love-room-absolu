...
function blocksDates(item: BookingRequest, now = new Date()) {
  ...
  return (
    item.status === "pending" && Date.parse(item.expiresAt) > now.getTime()
  );
}
...
export class LocalReservationRepository implements ReservationRepository {
  ...
  async findByReference(reference: string) {
    return (
      (await this.list()).find((item) => item.reference === reference) ?? null
    );
  }
  async findBetween(from: string, to: string) {
    return (await this.list()).filter((item) =>
      dateRangesOverlap(from, to, item.checkIn, item.checkOut),
    );
  }
  async occupiedRanges(now = new Date()) {
    return (await this.list())
      .filter((item) => blocksDates(item, now))
      .map((item) => ({ start: item.checkIn, end: item.checkOut }));
  }
  async isAvailable(checkIn: string, checkOut: string) {
    return !(await this.occupiedRanges()).some((range) =>
      dateRangesOverlap(checkIn, checkOut, range.start, range.end),
    );
  }
  async create(reservation: BookingRequest) {
    ...
    writeQueue = writeQueue.then(async () => {
      ...
      if (duplicate) {
        failure = new Error("DUPLICATE_REQUEST");
        return;
      }
      ...
      if (overlap) {
        failure = new Error("DATES_UNAVAILABLE");
        return;
      }
      ...
      await fs.writeFile(
        temporary,
        JSON.stringify([...current, reservation], null, 2),
        "utf8",
      );
