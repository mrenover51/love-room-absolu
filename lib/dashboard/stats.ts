...
export function buildStats(rows: DashboardReservation[], now: Date): DashboardStats {
  ...
  const previousMonthRevenue = revenue((r) => { const d = new Date(r.created_at); return d.getUTCFullYear() === previousMonthDate.getUTCFullYear() && d.getUTCMonth() === previousMonthDate.getUTCMonth(); });
  return { todayRevenue: revenue((r) => r.created_at.startsWith(today)), weekRevenue: revenue((r) => r.created_at >= day(startWeek)), monthRevenue: revenue((r) => { const d=new Date(r.created_at); return d.getUTCFullYear()===year&&d.getUTCMonth()===month }), yearRevenue: revenue((r) => new Date(r.created_at).getUTCFullYear() === year), previousMonthRevenue, reservations: rows.length, confirmedReservations: confirmed.length, cancelledReservations: rows.filter((r)=>r.status==="cancelled").length, occupancy: Math.min(100, Math.round(occupiedNights / (365) * 100)), averageBasket: paid.length ? Math.round(paid.reduce((s,r)=>s+r.total,0)/paid.length) : 0, averageStay: confirmed.length ? Math.round((confirmed.reduce((s,r)=>s+r.nights,0)/confirmed.length)*10)/10 : 0, pendingPayments: rows.filter((r)=>r.status==="pending_payment"||r.payment_status==="unpaid").length, pendingRequests: rows.filter((r)=>r.status==="pending").length };
}
