...
export class CheckoutService {
  ...
  private async createSession(reservation:{id:string;reference:string;expiresAt:string;pricing:PriceBreakdown},email:string,requestId?:string) {
    ...
    const session = await createReservationCheckoutSession({stripe:stripeProvider.getClient(),reservation:{...reservation,promoCode:reservation.pricing.promoCode},lineItems:checkoutLineItems(reservation.pricing),email,siteUrl,requestId});
